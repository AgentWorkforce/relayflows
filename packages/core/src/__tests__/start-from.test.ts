...
// so the DAG/skip/cache machinery is exercised without launching any processes.
...
describe('startFrom', () => {
  ...
  beforeEach(() => {
    ...
    db = makeDb();
    runner = new WorkflowRunner({ db, workspaceId: 'ws-test', cwd: tmpDir, executor: makeExecutor() });
  });

    ...
    expect(report.permissions).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          agent: 'writer',
          access: 'readwrite',
          // cloud v8 grants write on every non-denied file under readwrite
          // access (5 workspace files − 2 denied: .env, secrets/** = 3), where
          // the old compiler counted only the explicit files.write pattern (1).
          writePaths: 3,
        }),
        expect.objectContaining({
          agent: 'admin-lead',
